...
namespace AlphaECS {
    public class Group<T1, T2, T3, T4, T5, T6, T7> : IGroup, IDisposableContainer, IDisposable
        ...
        public IDisposable OnAdd(Action<IEntity, T1, T2, T3, T4, T5, T6, T7> action) {
            return Entities.ObserveAdd().Select(x => x.Value).StartWith(Entities).
                Subscribe(entity => action(entity, entity.Get<T1>(), entity.Get<T2>(), 
                    entity.Get<T3>(), entity.Get<T4>(), entity.Get<T5>(), 
                    entity.Get<T6>(), entity.Get<T7>()));
        }
        ...
        public void ForEach(Action<IEntity, T1, T2, T3, T4, T5, T6, T7> action) {
        ...
        public virtual void Initialize(IEventSystem eventSystem, IPoolManager poolManager) {
            Components = new Type[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4), typeof(T5), typeof(T6), typeof(T7) };
